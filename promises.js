...
function formatName(name) {
	return name.split('_') // ['john', 'smith']
		.map(n => n.charAt(0).toUpperCase() + n.slice(1)) // ['John', 'Smith']
		.join(' ') // 'John Smith'
}

function doFirstTask(data) {
	// filter out minors
	const newArr = data.filter((person) => person.age >= 18)
	...
	// format each name
	newArr.map(person => {
		return Object.assign({}, person, { fullName: formatName(person.fullName) })
	})
...
function doSecondTask(data) {
	return data.reduce((prev, person) => {
		return prev + person.salary
	}, 0)
}

const firstPromise = new Promise((resolve) => {
	resolve(doFirstTask(data))
})

const secondPromise = new Promise((resolve) => {
	resolve(doSecondTask(data))
})
